  ...
  const handleSaveEdit = async () => {
    try {
      const res = await fetch(`/api/admin/users/${editingUserId}`, {
        ...
        body: JSON.stringify({ username: editUsername, role: editRole, fullName: editFullName })
      });
                    ...
                    <td colSpan={4} className="px-6 py-4 border-b border-gray-200">
                      <div className="flex flex-col sm:flex-row gap-4 items-end">
                        <div className="flex-1">
                          ...
                          />
                          <p className="mt-1 text-xs text-amber-700/50">Minimum 8 chars, mixed case & numbers</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <button onClick={handleSavePassword} className="inline-flex items-center px-3 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-amber-600 hover:bg-amber-700">
                            <Save className="w-4 h-4 mr-1" /> Set Password
                          </button>
